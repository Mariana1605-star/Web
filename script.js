...
data-url="${escHtml(img.url_imagen)}">
    ...
    clone.addEventListener('slide.bs.carousel', (event) => {
        ...
        // Obtener el id de la imagen desde el data-id del slide
        const id = slide.dataset.id;

        console.log("Cambiando imagen al ID: " + id);

        // Llamada AJAX a datos.php para obtener los datos de la imagen por su id
        $.ajax({
            url:      'datos.php',
            type:     'GET',
            data:     { id: id },
            ...
            success: function(img) {
                // img es el objeto de la imagen devuelto por datos.php?id=X
                if (img && img.url_imagen) {
                    // Actualizar el src de la imagen del slide
    ...
    url_imagen:  slide.dataset.url,
            ...
            error: function() {
                ...
                // Fallback con datos ya en el DOM
                actualizarInfoImagen({
                    titulo:      slide.dataset.titulo,
                    descripcion: slide.dataset.descripcion,
                });
